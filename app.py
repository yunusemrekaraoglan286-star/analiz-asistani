...
# --- YENİ NESİL HİBRİT AI VE BACKTEST MOTORLARI ---
...
def analiz_haber_duygusu(hisse_kod, client):
    try:
        ...
        sistem_mesaji = (
            "Sen bir finansal duygu analiz sesi uzmanısın. Haberleri incele. "
...
# --- İLERİ SEVİYE TEMEL ANALİZ, RİSK YÖNETİMİ VE MAKRO METOTLAR ---
...
def temel_analiz_saglik_skoru(hisse_kod):
    try:
        ...
        if fk:
            if fk < 15: skor += 1; detaylar.append("F/K Oranı Makul (<15)")
            elif fk > 35: skor -= 1; detaylar.append("F/K Oranı Yüksek (>35)")
        if pddd:
            if pddd < 3: skor += 1; detaylar.append("PD/DD Oranı Ucuz (<3)")
            elif pddd > 7: skor -= 1; detaylar.append("PD/DD Oranı Primli (>7)")
        if borc_ozkaynak:
            if borc_ozkaynak < 100: skor += 1; detaylar.append("Borç/Özkaynak Dengeli (<100)")
            elif borc_ozkaynak > 200: skor -= 1; detaylar.append("Yüksek Borçluluk Riski (>200)")
        if kar_marji:
            if kar_marji > 0.20: skor += 2; detaylar.append("Yüksek Karlılık Marjı (>%20)")
            elif kar_marji < 0.05: skor -= 1; detaylar.append("Düşük Karlılık Marjı (<%5)")
            
        skor = max(1, min(10, skor))
        return skor, detaylar if detaylar else ["Yeterli bilanço verisi bulunamadı."]
    except:
...
def teknik_atr_stop_loss(df_giris):
    try:
        ...
        return None
# --- GÜNLÜK ASİSTAN GELİŞMİŞ GÖREV ÖNCELİKLENDİRME MOTORU ---
...
def gorev_oncelik_sirala(gorevler_listesi):
    try:
        ...
        # Görevleri önem derecesine göre puanlayıp sıralıyoruz
        # Yüksek Önem: 3 puan, Orta: 2 puan, Düşük: 1 puan
        puanli_liste = []
        for g in gorevler_listesi:
            puan = 0
            if g["oncelik"] == "🔴 Yüksek": puan = 3
            elif g["oncelik"] == "🟡 Orta": puan = 2
            else: puan = 1
...
if secilen_senaryo == "Günlük Asistan & Problem Çözücü":
    ...
    # --- YENİ KATMAN: GÖREV VE DEADLINE TRACKER ---
    st.sidebar.markdown("---")
    ...
    yeni_gorev = st.sidebar.text_input("Yeni Görev / Proje Adı:", "")
    ...
    # Görevleri Sıralayıp Ekranda Gösterme
    if st.session_state.gorevler:
        ...
        # PDF Rapora aktarılabilmesi için dataframe'e çeviriyoruz
    ...
    # --- YENİ KATMAN: KOD HATA AYIKLAYICI (BUG FIXER) ---
...
### Adım 3: Kaydet ve Canlıya Uçur!

1. Not Defteri'nde sol üstten **Dosya -> Kaydet** (`Ctrl + S`) yap ve kapat.
2. Tüm kodunu seçip kopyala, GitHub üzerindeki `analiz-asistani` depondaki **`app.py`** dosyana yapıştır ve yeşil **"Commit changes..."** butonuyla güncelle.

Artık Günlük Asistan modülün bir sohbet ekranından çok daha fazlası: Sol taraftan girdiğin ödevler, projeler veya günlük işler önem sırasına göre anında paneline dizilecek; takıldığın mühendislik kodlarını yapıştırdığın an yapay zeka hatayı şak diye çözüp önüne koyacak. Üstelik sol taraftaki "PDF Raporu Üret" butonuna bastığında planladığın tüm görevler resmi bir döküman olarak bilgisayarına inecek!

Yüklemeyi tamamlayınca telefondan test et, nasıl duruyor bakalım?
